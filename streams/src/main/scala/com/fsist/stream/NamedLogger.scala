...
/** A source-compatible replacement for the [[Logging]] trait that allows appending a string to the logger name by
  * calling the `named` method. The Logger object returned by `this.logger` is still a Typesafe Logging object whose
  * methods are macros.
  *
