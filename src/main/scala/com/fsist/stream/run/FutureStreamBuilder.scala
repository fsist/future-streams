...
class FutureStreamBuilder extends LazyLogging {
  ...
  private val runningStreamPromise = Promise[RunningStream]()
  ...
  /** Returns a Future that is fulfilled once `run` has been called.
    ...
    */
  def runningStream: Future[RunningStream] = runningStreamPromise.future
  ...
  def run()(implicit ec: ExecutionContext): RunningStream = {
    ...
    try {
      ...
      val stream = new RunningStream(this, componentMachines.mapValues(_.running), connectorMachines.mapValues(_.running), graphOps)
      runningStreamPromise.trySuccess(stream)
      stream
    }
    catch {
      case NonFatal(e) =>
        runningStreamPromise.tryFailure(e)
        throw e
    }
  }
